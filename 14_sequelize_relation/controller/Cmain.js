...
exports.getPlayer = async (req, res) => {
  try {
    ...
    const { playerId } = req.params; // 구조분해할당
    const player = await Player.findOne({
      where: {
        player_id: playerId,
      },
      include: [{ model: ProfileModel, attributes: ["position", "salary"] }],
...
exports.postPlayer = async (req, res) => {
  try {
    ...
    const newPlayer = await Player.create({
      // 보내려는 변수 name, DB 컬럼명도 name -> name: name, 형태가 아닌 name만 써도 됨
      name,
      age,
      teamid,
    });
...
// {playerId} req.params정보: 어떤 선수를
// {teamId} req.body정보: 어떤 팀으로 변경할건지
...
exports.deletePlayer = async (req, res) => {
  try {
    ...
    const isDeleted = await Player.destroy({
      where: {
        player_id: playerId, // db설정이름: 들어오는값이름
      },
    });
...
exports.getTeams = async (req, res) => {
  try {
    ...
    if (sort === "name_asc") {
      ...
      // sort나 search가 전달되지 않았을 때
      // team 테이블의 team_id, name 컬럼 보여주기
      // SELECT team_id, name FROM team
      teams = await Team.findAll({
        attributes: ["team_id", "name"], // 전체 조회(*)가 아니기 때문에 attributes를 써줘야 함
      });
    }
    // teams = [] or [{},{}]
...
exports.getTeamPlayers = async (req, res) => {
  try {
    // 1. 팀 정보는 안봐도 되고, 특정 팀의 선수 정보만 확인하기 위해서는 Player 모델에서 teamid 기준으로 조회
    // const { teamId } = req.params;
